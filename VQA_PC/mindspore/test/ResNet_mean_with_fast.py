...
class ResNet(nn.Cell):
    ...
    def construct(self, x, x_fast_features):
        ...
        x_size = x.shape #x.shape  of input: (1, 4, 3, 224, 224)
        x_fast_features_size = x_fast_features.shape #x_fast_features.shape  of input: (1, 4, 256)
        ...
        x = self.layer4(x)
        #x.shape  before avgpool: (4, 2048, 7, 7)
        x_avg = self.avgpool(x,[i for i in range(len(x.shape))][-2:])#x_avg.shape after avgpool: (4, 2048, 1, 1)
        # x = torch.flatten(x_avg, 1)
        x = ops.Flatten()(x_avg) #x.shape  after flatten: (4, 2048)
        # x = torch.cat((self.bn_img((self.adjust1(x))), self.bn_video(self.adjust2(x_fast_features))), dim=1)
        x = ops.cat((self.bn_img((self.adjust1(x))), self.bn_video(self.adjust2(x_fast_features))), axis=1)#x.shape  after cat: (4, 256)
