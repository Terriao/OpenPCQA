...
class VideoDataset_NR_image_with_fast_features():
    ...
    def __getitem__(self, idx):
        ...
        # transformed_video = ops.Zeros()((video_length_read, video_channel, video_height_crop, video_width_crop), mindspore.float32)
        transformed_video = np.zeros([video_length_read, video_channel, video_height_crop, video_width_crop], dtype=np.float32)
        video_read_index = 0
        ...
        # transformed_feature = ops.Zeros()((video_length_read, 256), mindspore.float32)
        ...
        for i in range(video_length_read):
            ...
            # feature_3D = Tensor.from_numpy(feature_3D)
            feature_3D = np.squeeze(feature_3D)
