...
class VideoDataset_NR_image_with_fast_features():
    ...
    def __init__(self, data_dir, data_dir_3D , datainfo_path, crop_size, epoch, batch_size, 
        is_train=True, frame_index=1, video_length_read = 4):
    ...
    def parse_item(self, idx):
        ...
        # transformed_video = np.zeros([video_length_read, video_height_crop, video_width_crop, video_channel], dtype=np.float32)#channel放最后是为了tf.image数据处理的需要，但TensorFlow的tensor不支持直接赋值
        video_read_index = 0
        for i in range(video_length_read):
            ...
            if os.path.exists(imge_name):
                ...
                # transformed_video[i] = read_frame[0]
                read_frame = tf.expand_dims(read_frame, 0)
                if(i == 0):
                    transformed_video = read_frame
                else:
                    transformed_video = tf.concat([transformed_video, read_frame], 0) #[n, 224, 224, 3]
        ...
        if video_read_index < video_length_read:
            for j in range(video_read_index, video_length_read):
                ...
                transformed_video = tf.concat([transformed_video, transformed_video[video_read_index-1:video_read_index]], 0)

        # read 3D features
        feature_folder_name = os.path.join(self.data_dir_3D, video_name.split('.')[0])
        # transformed_feature = np.zeros([video_length_read, 256], dtype=np.float32)
        for i in range(video_length_read):
