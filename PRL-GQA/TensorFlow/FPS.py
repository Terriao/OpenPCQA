...
def index_points(points, idx):    #将点集按照序号采样恢复成具体点坐标
    ...
    batch_indices = np.arange(B, dtype=np.int64).reshape(view_shape)
    batch_indices = np.repeat(batch_indices,repeat_shape[1],axis=1)
...
def square_distance(src, dst):
    ...
    dist += np.sum(src ** 2, axis=-1).reshape((B, N, 1))
    # dist += torch.sum(src ** 2, -1).view(B, N, 1)
    dist += np.sum(dst ** 2, axis=-1).reshape((B, 1, M))
    # dist += torch.sum(dst ** 2, -1).view(B, 1, M)
    return dist

def query_ball_point(radius, nsample, xyz, new_xyz):
    """
    为了尽可能多的将半径内的点采出，应尽量将点数设置大点，而半径小点，如果半径过大，点数太少，则取出的点会显的不再一个范围内。点数设置不应该超过总点数。
    Input:
        radius: local region radius
        nsample: max sample number in local region
        xyz: all points, [B, N, 3] B一般=1
        new_xyz: query points, [B, S, 3]      为new_xyz中每一点向xyz中点求取距离半径内点的序号，末尾序号为0的点代表超过半径外的点.最大点数下，可能所有点都小于半径，也可能部分大于半径
    Return:
        group_idx: grouped points index, [B, S, nsample] 包含的是new_xyz中每一点的radius半径内的邻近点的序号，序号数值的分布范围是0~N-1，如果没有一个点在radius内，数值就是N
    """
    # device = xyz.device
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
    group_idx = np.arange(N, dtype=np.int64).reshape((1, 1, N))
    group_idx = np.repeat(group_idx,S,axis=1)
    group_idx = np.repeat(group_idx,B,axis=0)
    ...
    group_idx = np.sort(group_idx,axis=-1)[:, :, :nsample]    #取排序后的结果 B x S x nsample 先按照行从小到大排序，半径之外的点肯定排在最后，且只取了最大点数值,半径范围内如果达到点数，则取最大点数，否则取半径外点以达到最大点数
    group_first = np.repeat(group_idx[:, :, 0].reshape((B, S, 1)),nsample,axis=-1)
...
def relative_cordinate(xyz,centroids):
    ...
    return xyz - centroids.reshape((B,S,1,C))
...
if __name__ == '__main__':
    ...
    '''
    points = np.arange(2*5*3, dtype=np.float32).reshape((2,5,3))
    idx = np.arange(2*2, dtype=np.int64).reshape((2,2))
    out = index_points(points, idx)
    print(out)
    '''

    np.random.seed(1)
    xyz = np.random.randn(1,6,3).astype(np.float32)
    new_xyz = np.random.randn(1,4,3).astype(np.float32)
    out = query_ball_point(radius=3, nsample=3, xyz=xyz, new_xyz=new_xyz)
    print(out)