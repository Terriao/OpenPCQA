...
from FPS import load_ply,farthest_point_sample,index_points,query_ball_point,relative_cordinate #,draw_points,KNN_sample
import numpy as np
...
class PointCloudDataset():
    ...
    def __getitem__(self, index):
        ...
        # data1_tensor = torch.tensor(result1_value, dtype=torch.float)#torch.float32(torch.float)
...
if __name__ == '__main__':
    ...
    print(len(PCDataset)) #12600
    trainloader = ds.GeneratorDataset(PCDataset, column_names=["data1_tensor", "data2_tensor", "label_tensor", "distortion_type"],
        num_parallel_workers=2, shuffle=True, python_multiprocessing=False)
    trainloader = trainloader.batch(1, drop_remainder=False)
    ...
    for i, input in enumerate(trainloader):
        print(input['data1_tensor'].shape) #(1, 64, 516, 3)
        print(input['data2_tensor'].shape) #(1, 64, 516, 3)
        print(input['label_tensor'].shape) #(1, 1)
        if input['distortion_type'].asnumpy().item() == 'com&down':
            print(input['distortion_type']) #['noise']
        break
